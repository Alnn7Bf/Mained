...
function ServiceCard({image, icon, title, description, longDescription, position}) {
  ...
  return (
    <>
      <div 
        className="relative overflow-hidden rounded-global cursor-pointer" 
        onClick={handleClick}
      >
        <div 
          ...
          style={
            { 
              ...
              backgroundPosition: `0% ${bgPosition}%` 
            }
          }
        ></div>
        <div 
          className={`
            ...
            items-center 
            bg-black/40 
            ${isOpen? 'md:bg-black/40 md:text-light' : 'md:bg-light md:text-black'} 
        ...
        >
          ...
          <div 
            className={`
              ...
              duration-300 
              ease-in 
              ${isOpen? "h-60 md:h-40 opacity-100" : "h-0 opacity-0"}
            `}/*aquí se deben de cambiar los tamaños de la altura según el texto que se coloque*/
          >
